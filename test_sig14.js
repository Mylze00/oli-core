...
async function testAll() {
    ...
    await tryPayload(payload, 'payment_c2b unsorted');

    // Also let's try /deposit unsorted
    ...
    try {
        ...
        console.log(`[deposit unsorted] SUCCESS:`, response.data);
    } catch (e) {
        let data = e.response ? e.response.data : e.message;
        if (typeof data === 'string' && data.includes('<p>Signature')) data = 'Signature is not valid';
        console.log(`[deposit unsorted] ERROR:`, data);
    }
