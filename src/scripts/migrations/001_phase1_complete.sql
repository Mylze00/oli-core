-- ====================================
-- OLI - Migration Phase 1 (FINAL)
-- Nouvelles tables et colonnes pour le super-app
-- ====================================
...
CREATE TABLE IF NOT EXISTS shops (
    ...
    owner_id UUID REFERENCES users(id) ON DELETE CASCADE, -- Users est UUID
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS shop_id INTEGER REFERENCES shops(id) ON DELETE SET NULL; -- Shops est SERIAL
...
CREATE TABLE IF NOT EXISTS delivery_orders (
    id SERIAL PRIMARY KEY,
    order_id INTEGER REFERENCES orders(id) ON DELETE CASCADE, -- Orders est INTEGER (vu erreur précédente)
    deliverer_id UUID REFERENCES users(id) ON DELETE SET NULL, -- Users est UUID
...
CREATE TABLE IF NOT EXISTS wallet_transactions (
    id SERIAL PRIMARY KEY,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE, -- Users est UUID
...
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS product_id INTEGER REFERENCES products(id) ON DELETE SET NULL; -- Pari: Products est INTEGER

-- 7. Enrichir la table messages
ALTER TABLE messages ADD COLUMN IF NOT EXISTS reply_to_id INTEGER REFERENCES messages(id) ON DELETE SET NULL; -- Pari: Messages est INTEGER
...
CREATE TABLE IF NOT EXISTS favorites (
    id SERIAL PRIMARY KEY,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE, -- Users est UUID
    product_id INTEGER REFERENCES products(id) ON DELETE CASCADE, -- Products est INTEGER
    created_at TIMESTAMP DEFAULT NOW(),
