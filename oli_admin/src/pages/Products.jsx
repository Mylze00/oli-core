...
export default function Products() {
    ...
    const toggleFeatured = async (product) => {
        ...
        } catch (error) { console.error("Erreur toggle featured:", error); alert("Erreur"); }
    };
    ...
    const handleDelete = async (productId) => {
        ...
        } catch (error) { console.error("Erreur delete:", error); alert("Erreur"); }
    };
    ...
    const toggleGoodDeal = async (product) => {
        ...
        try {
            ...
            setProducts(products.map(p => p.id === product.id ? { ...p, is_good_deal: newVal, promo_price: newVal ? newPrice : null } : p));
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-50">
                        {filteredProducts.map((product) => (
                            <div key={product.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-gray-50 transition">
                                {/* Produit */}
                                ...
                                <div className="col-span-1 text-center">
                                    ...
                                    {product.is_good_deal && product.promo_price && (
                                        <p className="text-xs text-rose-500 font-medium">{product.promo_price} $</p>
                                    )}
                                </div>
                                ...
                                <div className="col-span-1 text-center flex flex-col items-center gap-1">
                                    {product.is_featured && (
                                        <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full font-medium flex items-center">
                                            <StarIcon className="h-3 w-3 mr-0.5" />Vedette
                                        </span>
                                    )}
                                    {product.is_good_deal && (
                                        <span className="text-xs text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full font-medium flex items-center">
                                            <FireIcon className="h-3 w-3 mr-0.5" />Deal
                                        </span>
                                    )}
                                    {!product.is_featured && !product.is_good_deal && (
