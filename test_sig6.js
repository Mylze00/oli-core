...
async function tryPayload(field) {
    const payload = {
        merchant_id: UNIPESA_MERCHANT,
        order_id: 'test-F1-' + field,
        amount: '500',
        currency: 'CDF',
...
async function testAll() {
    const fields = [
        'customer_name', 'customer_firstname', 'customer_lastname', 'email', 'customer_email',
        'provider', 'network', 'channel', 'method', 'type',
        'callback_url', 'return_url', 'webhook_url', 'notify_url',
        'lang', 'language', 'device', 'ip', 'metadata', 'extra'
    ];
